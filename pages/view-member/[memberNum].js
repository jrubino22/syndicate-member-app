...
const MemberPage = ({ member }) => {
  ...
  async function handleFormSubmit() {
    const thisUrl = `https://member.vsyndicate.com/api/update/${urlParam}`;
  ...
  async function postFormDataAsJson() {
    ...
    const response = await fetch(
      `https://member.vsyndicate.com/api/update/${member[0].accountNumber}`,
      fetchOptions
    );
  ...
  return member.map(({ customerEmail, cardTier, accountNumber, notes }) => (
    <>
      <Link href={`/`}>
      ...
      <div className="border">
        <p className="memberPageHeader">
          <span>
            <span key={customerEmail} className="bold">
              Customer Email:
            </span>{' '}
            {customerEmail}{' '}
          </span>
          <span>
            <span key={accountNumber} className="bold">
              | Account Number:
            </span>{' '}
            {accountNumber}{' '}
          </span>
          <span>
            <span key="cardTier" className="bold">
...
export async function getServerSideProps(props) {
  const response = await fetch(
    `https://member.vsyndicate.com/api/member/${props.query.memberNum}`
  );
